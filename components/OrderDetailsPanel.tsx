...
import { Printer, RefreshCw, AlertTriangle, Truck, MapPin, CreditCard, User, Package, ExternalLink, ArrowLeft, FileText, Settings2 } from 'lucide-react';
...
const OrderDetailsPanel: React.FC<OrderDetailsPanelProps> = ({ order, onOrderUpdated, onCloseMobile }) => {
  ...
  if (!order) {
    return (
      <div className="hidden md:flex h-full flex-col items-center justify-center text-slate-400 dark:text-slate-500 p-8 border border-slate-200 dark:border-slate-800 border-dashed rounded-xl bg-slate-50 dark:bg-slate-900/50">
        ...
        <p className="text-lg font-medium text-slate-500 dark:text-slate-400">No Order Selected</p>
  ...
  const handleGenerateLabel = async () => {
    ...
    try {
      ...
      // 1. Call Sendify Service (Generate Label)
      // Use the custom ID if provided, otherwise default to internal Order ID (which likely returns dummy if not mapped)
      const labelUrl = await sendifyService.generateLabel(order.id, customShipmentId);
      
      // 2. Update Order via OrderService
      ...
      showNotification('success', 'Shipping label generated successfully.');
      ...
      setIsPrinting(false);
      showNotification('error', error.message || "Error generating shipping label. Please check the shipment status.");
    }
  };

  // Logic to display address from Snapshot (SQL Schema) or fallback to User Address
  const renderAddress = () => {
    if (order.shippingAddressSnapshot) {
        return (
            <div className="space-y-0.5">
                <p>{order.shippingAddressSnapshot}</p>
                <p>{order.shippingCitySnapshot}, {order.shippingStateSnapshot} {order.shippingZipSnapshot}</p>
                <p>{order.shippingCountrySnapshot}</p>
            </div>
        );
    }
    return <p className="whitespace-pre-line leading-relaxed">{order.user?.address}</p>;
  };
    ...
    <div className="bg-white dark:bg-slate-900 md:border md:border-slate-200 dark:md:border-slate-800 md:rounded-xl shadow-sm flex flex-col h-full w-full transition-colors">
      ...
      <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex flex-col gap-3 bg-slate-50/50 dark:bg-slate-800/30">
        ...
        <div className="flex md:hidden items-center mb-2">
            <button onClick={onCloseMobile} className="flex items-center text-slate-500 hover:text-indigo-600 dark:text-slate-400 dark:hover:text-indigo-400">
        ...
        <div className="flex justify-between items-start">
            ...
            <div className="flex items-center gap-2 mb-1">
                <h2 className="text-2xl font-bold text-slate-800 dark:text-white">{order.orderNumber}</h2>
            ...
            <div className="flex flex-col items-end gap-2">
                {confirmReprint && (
                    <div className="flex flex-col items-end mb-2 animate-in fade-in slide-in-from-top-1">
                        ...
                        <button onClick={() => setConfirmReprint(false)} className="text-xs text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 underline">
                        Cancel
                        </button>
                    </div>
                )}
                ...
                <button
                    ...
                    `}
                >
                    {isPrinting ? <RefreshCw size={18} className="animate-spin" /> : <FileText size={18} />}
                    <span>{isPrinting ? 'Generating...' : (confirmReprint ? 'Regenerate' : 'Generate Label')}</span>
                </button>
        ...
        {/* Advanced / Debugging Options for Sendify */}
        {showAdvancedLabel && (
            <div className="mt-2 p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg animate-in fade-in zoom-in-95">
                <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">
                    Manual Sendify Shipment ID (Testing)
                </label>
                ...
                <p className="text-[10px] text-slate-400 mt-1">
                    Paste a real Shipment ID from your Sendify Dashboard to test printing. Leave empty to use Order ID ({order.id}).
                </p>
            </div>
        )}
        ...
        {order.labelUrl && (
            <a href={order.labelUrl} target="_blank" rel="noreferrer" className="text-xs flex items-center text-indigo-600 dark:text-indigo-400 hover:underline">
        ...
        <div>
          ...
          <div className="space-y-4">
            {order.orderItems.map((item) => (
              <div key={item.id} className="flex gap-4 p-3 hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-lg border border-transparent hover:border-slate-100 dark:hover:border-slate-700 transition-colors">
                <AsyncImage src={item.product?.imageUrl} alt={item.product?.name} className="w-16 h-16 rounded-md border border-slate-200 dark:border-slate-700 shrink-0" />
                <div className="flex-1 min-w-0">
                  <h4 className="text-sm font-medium text-slate-900 dark:text-white truncate">{item.product?.name}</h4>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">{item.product?.inspiredBy ? `Inspired by ${item.product.inspiredBy}` : item.product?.description}</p>
                  <div className="flex items-center gap-2 mt-1">
                     <span className="text-xs font-mono bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded text-slate-600 dark:text-slate-300">Qty: {item.quantity}</span>
                     <span className="text-xs text-slate-500 dark:text-slate-400">x €{item.unitPrice.toFixed(2)}</span>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">€{(item.quantity * item.unitPrice).toFixed(2)}</p>
                </div>
              </div>
            ))}
          </div>
